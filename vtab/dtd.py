...
class DTD(VisionDataset):
    ...
    def __init__(
        ...
        classes_aux = []
        if type == 'train1000':
            with open(self._meta_folder / f"val{self._partition}.txt") as file:
                for line in file:
                    cls, name = line.strip().split("/")
                    image_files_aux.append(self._images_folder.joinpath(cls, name))
                    classes_aux.append(cls)
        ...
        if type == 'train1000':
            ...
            classes = classes[:800] + classes_aux[:200]
        elif type == 'train800':
